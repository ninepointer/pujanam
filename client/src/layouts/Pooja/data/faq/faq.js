...
const AddTier = ({ prevData, faq }) => {
    ...
    async function deleteFaq(id){
        const res = await fetch(`${apiUrl}pooja/deletefaq/${data?._id}`, {
            method: "DELETE",
            credentials: "include",
            headers: {
    ...
    data?.faq?.map((elem) => {
        ...
        infoData.edit = (
            <AiOutlineEdit onClick={() => { setCreateForm(true); setId(elem) }} cursor="pointer" />
        );
        infoData.delete = (
            <DeleteIcon onClick={() => { deleteFaq(elem?._id) }} cursor="pointer" />
        );
        infoData.question = (
    ...
    return (
        ...
        <Card>
            ...
            {createForm && <>
                <CreateFaq createForm={createForm} setCreateForm={setCreateForm} prevData={prevData} prevFaq={id} faq={faq} setId={setId}/>
            </>
            }
            <MDBox mt={1}>
                <DataTable
