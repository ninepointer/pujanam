...
const ActiveMandir = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`${apiUrl}mandir/inactive/?skip=${skip}&limit=${limitSetting}`, {
      ...
      {data.length > 0 ?
        ...
        <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography fontFamily='Itim' color="success">No mandir(s) in active state</MDTypography>
          </Grid>
        </Grid>
      }
