...
const ActiveMandir = () => {
  ...
  useEffect(() => {
    let call1 = axios.get(`${apiUrl}mandir/inactive/?skip=${skip}&limit=${limitSetting}`, {
  ...
  function backHandler() {
    ...
    setData([]);
    setIsLoading(true)
    axios.get(`${apiUrl}mandir/inactive/?skip=${skip}&limit=${limitSetting}`, {
      withCredentials: true,
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
      ...
      {data.length > 0 ?
        ...
        <MDBox>
                            ...
                            <Grid container lg={12} md={4} xs={12} display='flex' justifyContent='flex-start' alignItems='center' >
                              <Grid item xs={12} md={4} lg={12} display='flex' justifyContent='flex-start' style={{ maxWidth: '100%', height: 'auto' }}>
                                ...
                                <MDTypography variant="h6" fontSize={10} fontWeight={400} style={{ position: 'absolute', top: 0, right: 0, margin: '8px', textAlign: 'center', color: 'black', backgroundColor: "white", borderRadius: "15px", padding: "2px 10px 2px 10px", marginTop: "10px", fontFamily: 'Itim' }}>
                                  Views : {elem?.viewCount ? elem?.viewCount : 0}, {elem?.address_details?.state}
                                </MDTypography>
                              </Grid>
                              <Grid item xs={12} md={4} lg={12} display='flex' justifyContent='flex-start' style={{ maxWidth: '100%', height: 'auto' }}>
          ...
          {!isLoading && count !== 0 &&
            <MDBox mt={2} display="flex" justifyContent="space-between" alignItems='center' width='100%'>
              ...
              <MDTypography color="success" fontSize={15} fontWeight='bold'>Total Mandir: {!count ? 0 : count} | Page {(skip + limitSetting) / limitSetting} of {!count ? 1 : Math.ceil(count / limitSetting)}</MDTypography>
        ...
        <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography fontFamily='Itim' color="success">No mandir(s) in inactive state</MDTypography>
          </Grid>
        </Grid>
      }
