...
const DispatchModal = ({ data, withdrawalRequestDate, amount, withdrawalId, action, setAction }) => {
  const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
  ...
  const handleSubmit = async () => {
    ...
    const res = await axios.patch(`${apiUrl}order/deliver/${data?._id}`, formData, { withCredentials: true });
  ...
  return (
    <>
      <MDButton onClick={() => { setOpen(true) }} color='success' sx={{ marginRight: '6px' }}>Deliver</MDButton>
        ...
        <Box sx={style}>
          ...
          <MDBox mt={1} >
            ...
            <MDBox sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '12px' }}>
              ...
              <MDButton color='success' onClick={() => { handleSubmit() }}>Deliver</MDButton>
            </MDBox>
          </MDBox>
        </Box>
      </Modal>
