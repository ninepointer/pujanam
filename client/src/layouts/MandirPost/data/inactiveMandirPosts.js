...
const [data,setData] = useState([]);
  ...
  useEffect(()=>{
    let call1 = axios.get(`${apiUrl}mandirpost/inactive/?skip=${skip}&limit=${limitSetting}`,{
  ...
  function backHandler(){
    ...
    setData([]);
    setIsLoading(true)
    axios.get(`${apiUrl}mandirpost/inactive/?skip=${skip}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
      ...
      {data.length > 0 ?
                            ...
                            <CardActionArea 
                              ...
                              <Grid container lg={12} md={4} xs={12} display='flex' justifyContent='center' alignItems='center' >
                                <Grid item xs={12} md={4} lg={4} display='flex' justifyContent='center' style={{ maxWidth: '100%', height: 'auto' }}>
                                  <MDAvatar
                                      src={elem?.icon?.url ? elem?.icon?.url : DefaultPanditImage}
                                      alt={elem?.name}
                                      size="lg"
         ...
         <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography color="light">No Inactive Mandir Post(s)</MDTypography>
          </Grid>
         </Grid>
         } 
