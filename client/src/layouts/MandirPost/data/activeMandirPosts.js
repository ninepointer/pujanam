...
const [data,setData] = useState([]);
  ...
  useEffect(()=>{
    let call1 = axios.get(`${apiUrl}mandirpost/active/?skip=${skip}&limit=${limitSetting}`,{
  ...
  function backHandler(){
    ...
    setData([]);
    setIsLoading(true)
    axios.get(`${apiUrl}mandirpost/active/?skip=${skip}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
      ...
      {data.length > 0 ?
         ...
         <Grid container spacing={1} xs={12} md={6} lg={12}>
          <Grid item mt={2} xs={6} md={3} lg={12} display="flex" justifyContent="center">
            <MDTypography color="light">No Active Mandir Post(s)</MDTypography>
          </Grid>
         </Grid>
         } 
