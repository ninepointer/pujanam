...
const ApproveModal = ({ data, action, setAction }) => {
  ...
  const handleSubmit = async () => {
    ...
    const res = await axios.patch(`${apiUrl}consultation/confirm/${data?._id}`, formData, { withCredentials: true });
    ...
    <>
      ...
      >
        <Box sx={style}>
          ...
          <MDBox mt={1} >
            <MDTypography style={{ fontSize: '14px' }}>Confirm consultation for {data?.full_name}</MDTypography>
