...
const RejectModal = ( {open, handleClose, data, action, setAction}) => {
    ...
    const handleSubmit = async() =>{
        ...
        const res = await axios.patch(`${apiUrl}consultation/reject/${data._id}`, 
        {rejectionReason}, {withCredentials:true});
        console.log(res.data);
        if(res.data.status == 'success'){
            openSuccessSB('Success', 'Consultation rejected.');
            setAction(!action);
            handleClose();
        } else{
    ...
    return (
     ...
     >
      <Box sx={style}>
        <MDTypography>Reject Consultation</MDTypography>
        <MDBox mt={1} style={{height:'85vh', display:'flex', flexDirection:'column'}}>
          <MDTypography style={{fontSize:'14px', marginBottom:'12px'}}>Reject consultation request by {data.full_name}</MDTypography>  
