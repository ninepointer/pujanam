...
export default function CreateOtherName({ setId, createForm, setCreateForm, prevData, prevName, deviDev }) {
    ...
    const [formState, setFormState] = useState({
        name: "" || prevName?.other_names?.name,
        _id: "" || prevName?.other_names?._id
    });
    ...
    async function onNext(e, formState) {
        ...
        if (!formState?.other_names) {
            setTimeout(() => {  setIsSubmitted(false) }, 500)
            return openErrorSB("Missing Field", "Please fill all the mandatory fields")
        }

        const { other_names} = formState;

        const res = await fetch(`${apiUrl}devta/othername/${prevData?._id}`, {
            method: "PATCH",
            credentials: "include",
            headers: {
            ...
            body: JSON.stringify({
                other_names, prevPackage: prevName
            })
        });
                            ...
                            <Grid item xs={12} md={6} xl={5}>
                                <FormControl sx={{ width: '100%' }}>
                                    ...
                                    <Select
                                        ...
                                        value={formState?.other_names?.name || prevName?.name}
