...
const [data,setData] = useState([]);
  ...
  useEffect(()=>{
    let call1 = axios.get(`${apiUrl}devta/inactive/?skip=${skip}&limit=${limitSetting}`,{
  ...
  function backHandler(){
    ...
    setData([]);
    setIsLoading(true)
    axios.get(`${apiUrl}devta/inactive/?skip=${skip}&limit=${limitSetting}`,{
        withCredentials: true,
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
            ...
            <Grid container spacing={2} bgColor="light" display="flex" justifyContent="flex-start" alignItems='center'>
              {data?.map((elem, index)=>{
                    ...
                    return (
                      ...
                      <Grid key={elem?._id} item xs={12} md={4} lg={2} display='flex' justifyContent='center' alignContent='center' alignItems='center' style={{width: '100%',height: 'auto'}}>
                            ...
                            <CardContent display='flex' justifyContent='flex-start' style={{ maxWidth: '100%', height: 'auto' }}>
                                ...
                                <MDBox display='flex' justifyContent='flex-start' style={{ maxWidth: '100%', height: 'auto' }}>
                                    <MDTypography variant="caption" style={{ textAlign: 'justify', fontFamily: 'Itim' }}>
                                        {limitStringWithEllipsis(elem?.description,25)}
                                    </MDTypography>
                                </MDBox>
