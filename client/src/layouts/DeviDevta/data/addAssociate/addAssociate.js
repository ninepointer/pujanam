...
const AddTier = ({ prevData, deviDev }) => {
    ...
    async function deleteTier(id){
        const res = await fetch(`${apiUrl}devta/deleteassociate/${data?._id}`, {
            method: "DELETE",
            credentials: "include",
            headers: {
    ...
    data?.associated_devi_devta?.map((elem) => {
        let infoData = {}
    ...
    return (
        ...
        <Card>
            ...
            {createForm && <>
                <CreateAssociate createForm={createForm} setCreateForm={setCreateForm} prevData={prevData} prevName={id} deviDev={deviDev} setId={setId}/>
            </>
            }
            <MDBox mt={1}>
                <DataTable
